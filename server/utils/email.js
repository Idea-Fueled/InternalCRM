...
/**
 * Sends a welcome email with a password setup link using Resend.
 */
export const sendWelcomeEmail = async (toEmail, userName, setupLink) => {
    const resend = getResendInstance();
...
<head>
  <meta charset="UTF-8"/>
  <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
  <title>Welcome to IdeaFueled CRM</title>
</head>
<body style="margin:0;padding:0;background-color:#0f172a;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;-webkit-font-smoothing:antialiased;-moz-osx-font-smoothing:grayscale;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background-color:#0f172a;padding:64px 16px;min-height:100vh;">
    <tr>
        ...
        <table width="100%" style="max-width:540px;" cellpadding="0" cellspacing="0">
          ...
          <tr>
            <td style="background-color:#1e293b;border-radius:24px;border:1px solid rgba(255,255,255,0.08);box-shadow:0 20px 40px -15px rgba(0,0,0,0.5);padding:48px 40px;">
              ...
              <!-- Welcome Icon with Gradient Glow -->
              <table align="center" cellpadding="0" cellspacing="0" style="margin-bottom:28px;">
                <tr>
                  <td align="center" style="background:linear-gradient(135deg,#1e1b4b,#1e293b);border:1px solid rgba(59,130,246,0.3);border-radius:50%;width:72px;height:72px;line-height:72px;font-size:32px;text-align:center;box-shadow:0 0 20px rgba(59,130,246,0.15);">
                    🚀
                  </td>
                </tr>
              </table>
              ...
              <!-- Greeting & Subtitle -->
              <h2 style="margin:0 0 12px;font-size:24px;font-weight:700;color:#ffffff;text-align:center;letter-spacing:-0.5px;">Welcome to the Team!</h2>
              <p style="margin:0 0 32px;font-size:15px;color:#94a3b8;line-height:1.7;text-align:center;">
                Hi <strong style="color:#ffffff;">${userName}</strong>,<br/>
                An account has been created for you on <strong style="color:#ffffff;">IdeaFueled CRM</strong>. We're excited to have you on board! To get started, please set up your account password using the secure button below.
              </p>
              ...
              <table align="center" cellpadding="0" cellspacing="0" style="margin-bottom:32px;">
                <tr>
                  <td align="center">
                    <a href="${setupLink}" target="_blank" style="display:inline-block;background:linear-gradient(135deg,#3b82f6,#6366f1);color:#ffffff;text-decoration:none;font-weight:700;font-size:15px;padding:16px 40px;border-radius:14px;box-shadow:0 10px 20px -10px rgba(59,130,246,0.5);letter-spacing:0.2px;">
                      Set Up Password
                    </a>
                  </td>
                </tr>
              ...
              <!-- Expiry Notification -->
              <table width="100%" cellpadding="0" cellspacing="0" style="background-color:rgba(59,130,246,0.06);border:1px solid rgba(59,130,246,0.15);border-radius:14px;margin-bottom:32px;">
                <tr>
                  <td align="center" style="padding:14px 18px;font-size:13px;color:#60a5fa;font-weight:500;line-height:1.5;">
                    ⏱️ For security, this password setup link is valid for <strong>24 hours</strong>.
                  </td>
                </tr>
              </table>
              ...
              <p style="margin:0;font-size:12px;color:#64748b;line-height:1.6;word-break:break-all;">
                Button not working? Copy and paste this URL into your browser:<br/>
                <a href="${setupLink}" target="_blank" style="color:#6366f1;text-decoration:none;word-break:break-all;">${setupLink}</a>
              </p>
...
`;
    ...
    console.log(`[email] Sending welcome & password setup email via Resend to ${toEmail}...`);

    const { data, error } = await resend.emails.send({
        from,
        to: toEmail,
        subject: '👋 Welcome to Idea Fueled CRM – Set Up Your Account',
        html,
    });

    if (error) {
        console.error('[email] Resend sending welcome failed:', error);
        throw new Error(error.message || 'Failed to send welcome email via Resend.');
    }

    console.log(`[email] Welcome email sent successfully via Resend. ID: ${data?.id}`);
    return data;
};
